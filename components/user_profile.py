...
def create_user_profile_component():
    ...
    return dbc.Card([
        ...
        dbc.Popover(
                        [
                            html.P("Set your risk tolerance (1-10) and horizon."),
                            dbc.Button("Learn More", id={"type": "help-popover-link", "topic": "user_profile"}, color="link", size="sm")
                        ],
                        target="help-user-profile",
                        body=True,
                        trigger="legacy", # Click to open/close
                    )
    ], className="mb-4")
